...
//           "http://localhost:8000/api/generate/cta-score",
//           { imageUrl, variantId: dbRecordId },
//           { headers: { Authorization: `Bearer ${token}` } },
//         );
//         if (data.success) {
//           setctaScore(data.overall);
//           // you can also store data.data.metrics / data.data.recommendations
//           // and render them instead of (or alongside) the static dial text
//         }
//       } catch (err) {
//         console.error("CTA score fetch failed:", err);
//       } finally {
...
//     setAiFeedback(null);
//     try {
//       const response = await api.post("/api/generate/cta-score", {
//         imageUrl,
...
import { HiOutlineSparkles } from "react-icons/hi";
import { RiLoader4Line } from "react-icons/ri";
import api from "../api/apiInstance.js"; // 🌟 FIXED: Standard Interceptor imported
...
export default function CtaScore() {
  ...
  // States for Gemini Vision AI Doctor
  const [aiFeedback, setAiFeedback] = useState(null);
  const [isAiLoading, setIsAiLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchScore = async () => {
      ...
      setIsAnalyzing(true);
      try {
        // FIXED: Using standard api instance instead of raw axios
  ...
  const handleAiAuditRequest = async () => {
    ...
    setAiFeedback(null);
    try {
      // 🌟 FIXED: Using standard api instance to securely pass through cookies/tokens
      ...
      if (data.success) {
        ...
        setAiFeedback("Failed to extract diagnostic data matrix.");
      }
    } catch (error) {
      console.error("AI Doctor core validation pipeline broke down:", error);
      setAiFeedback(
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4 pt-20 text-slate-800 antialiased font-sans">
        <div className="w-full max-w-4xl bg-white border border-slate-200/80 rounded-3xl shadow-2xl flex flex-col md:flex-row overflow-hidden relative">
          {/* LEFT AREA: ASSET MONITOR VIEWPORT */}
          <div className="flex-1 bg-slate-900 p-6 flex flex-col justify-between relative group">
            <button
              onClick={() => navigate(-1)}
              className="absolute top-4 left-4 z-10 flex items-center gap-1 px-3 py-1.5 bg-black/40 hover:bg-black/60 text-white rounded-xl text-xs font-bold backdrop-blur-md border border-white/10 transition cursor-pointer"
            >
              <MdArrowBack size={14} /> Back
            </button>
            <div className="my-auto aspect-video rounded-2xl overflow-hidden border border-white/10 shadow-inner relative bg-slate-950 flex items-center justify-center">
              {imageUrl ? (
                <img
                  src={imageUrl}
            ...
            <div className="pt-4 border-t border-white/10 flex flex-col gap-0.5">
              ...
              <h2 className="text-sm font-black text-white tracking-tight line-clamp-1">
                {title || "Untitled Blueprint"}
              </h2>
            </div>
          </div>

          {/* RIGHT AREA: ANALYTICS METRIC CONTROLS */}
          <div className="w-full md:w-96 p-6 border-t md:border-t-0 md:border-l border-slate-200/80 flex flex-col justify-between bg-white">
            <div className="space-y-6">
              <div className="flex items-center gap-1.5 pb-3 border-b border-slate-100">
                <div className="p-1.5 bg-pink-50 rounded-xl text-pink-600">
                  <MdInsights size={18} />
                </div>
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 font-medium">
                    Neural network evaluation indexing matrix
                  </p>
                </div>
              </div>

              {/* GAUGES PANEL */}
              <div className="bg-slate-50/50 border border-slate-100 rounded-2xl p-4 flex flex-col items-center text-center relative overflow-hidden">
                {isAnalyzing ? (
                  ...
                  <>
                    <div className="relative flex items-center justify-center mb-2">
                      ...
                      <span className="text-xs font-black text-slate-400 absolute -bottom-1 font-mono">
                        /100
                      </span>
                    </div>
                    <div className="flex items-center gap-1 text-xs font-black uppercase tracking-wider mt-2">
                      {ctaScore >= 80 ? (
                        <span className="text-emerald-600 bg-emerald-50 px-2.5 py-1 rounded-lg flex items-center gap-0.5">
                          <MdCheckCircle /> High CTR Potential
                        </span>
                      ) : (
                        <span className="text-amber-600 bg-amber-50 px-2.5 py-1 rounded-lg flex items-center gap-0.5">
                          <MdWarning /> Low Contrast Warning
                        </span>
                      )}
                    </div>
                  </>
              ...
              {/* AI DOCTOR ACTION SECTION */}
              <div className="space-y-2.5">
                {!aiFeedback && !isAiLoading && (
                  <button
                    onClick={handleAiAuditRequest}
                    className="w-full flex cursor-pointer items-center justify-center gap-1.5 py-2.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-black uppercase tracking-wider rounded-xl transition shadow-md shadow-purple-600/10"
                  >
                    <MdAutoAwesome size={14} /> Diagnostic Audit through Gemini
                  </button>
                )}

                {isAiLoading && (
                  <div className="w-full py-4 bg-purple-50/50 border border-purple-100 rounded-xl flex flex-col items-center justify-center gap-2">
                    <RiLoader4Line
                      size={24}
                      className="text-purple-600 animate-spin"
                    />
                    <span className="text-[10px] font-mono font-black uppercase tracking-wider text-purple-400 animate-pulse">
                      Running Vision Core Models...
                    </span>
                  </div>
                )}
                ...
                {!isAiLoading && aiFeedback && (
                  <div className="bg-purple-50/40 border border-purple-100 rounded-xl p-4 space-y-3">
                    <div className="text-[10px] font-black text-purple-600 uppercase tracking-widest flex items-center gap-1">
                      <HiOutlineSparkles /> Live Diagnostics Report:
                    </div>
                    <div className="text-[11px] text-slate-700 leading-relaxed font-medium whitespace-pre-wrap font-sans bg-white p-3 rounded-lg border border-purple-200/40 shadow-xs max-h-[180px] overflow-y-auto">
                      {aiFeedback}
                    </div>
