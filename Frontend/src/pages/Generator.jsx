        ...
        <aside className="w-80 bg-white border-r border-slate-200 flex flex-col justify-between p-5 shrink-0 h-full overflow-y-auto">
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate("/")}
                className="p-1.5 hover:bg-slate-100 rounded-lg cursor-pointer text-slate-500 hover:text-slate-900 transition"
              >
                <MdArrowBack size={16} />
              </button>
              {/* <div
                className="flex items-center gap-2 font-black text-xl text-pink-600 tracking-tight cursor-pointer font-syne"
                onClick={() => navigate("/")}
              >
                <img
                  src={logoImg}
            ...
            <form onSubmit={handleGenerateSubmit} className="space-y-5">
              ...
              >
                {isGenerating ? (
                  ...
                  <>
                    <MdAutoAwesome size={14} className="text-pink-400" /> Render
                    Variations
                  </>
                )}
              </button>
          ...
          {isGenerating && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 h-full max-h-[85vh]">
              {[1, 2, 3, 4].map((id) => (
                <div
                  key={id}
                  className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col justify-between animate-pulse shadow-sm"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {generatedThumbnails.map((item) => (
                  <div
                    key={item._id}
                    className="bg-white border border-slate-200 rounded-xl p-4 flex flex-col justify-between shadow-sm hover:border-slate-300 transition-all group"
                  >
