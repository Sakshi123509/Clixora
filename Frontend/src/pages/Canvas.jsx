...
const AVAILABLE_FONTS = [
  ...
  { name: "Space Grotesk (Startup Vibe)", value: "'Space Grotesk', sans-serif" },
...
export default function Canvas() {
  ...
  const [selectedFont, setSelectedFont] = useState("Impact, Charcoal, sans-serif");
  ...
  useEffect(() => {
    ...
    link.href = "https://fonts.googleapis.com/css2?family=Anton&family=Archivo+Black&family=Bebas+Neue&family=Montserrat:wght=800;900&family=Oswald:wght=700&family=Poppins:wght=800;900&family=Space+Grotesk:wght=700&family=Syne:wght=800&family=Teko:wght=700&display=swap";
    link.rel = "stylesheet";
  ...
  }, [textLayers, selectedLayerId, imageUrl, brightness, saturation, selectedFont]);
  ...
  const handleCanvasMouseMove = (e) => {
    ...
    setTextLayers((prev) =>
      prev.map((l) => (l.id === selectedLayerId ? { ...l, x: mouseX, y: mouseY } : l))
    );
  };
  ...
  const handleSaveToDashboard = async () => {
    if (!dbRecordId) return alert("Invalid database session context.");
    setIsSaving(true);
    try {
      const canvas = canvasRef.current;
      ...
      if (response.data.success) {
        ...
        alert(response.data.error || "Save operation failed.");
      }
    } catch (error) {
      console.error(error);
      alert("Network pipeline broken.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-slate-100 text-slate-800 font-sans antialiased overflow-hidden">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder={selectedLayerId ? "Edit highlighted element..." : "Type phrase layer..."}
                  rows={2}
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-wide flex items-center gap-1">
                  <MdFontDownload size={12} /> Font Style
                </label>
                <select
                  value={selectedFont}
                  onChange={(e) => {
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-wide">Font Size</label>
                  <input
                    type="number"
                    value={textSize}
                    onChange={(e) => {
                      setTextSize(e.target.value);
                      updateSelectedLayerProperties("size", Number(e.target.value));
                    }}
                    className="w-full px-2 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs font-bold text-center"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-wide">Text Color</label>
              ...
              {/* BACKGROUND BOX */}
              <div className="bg-slate-50/80 p-2.5 border border-slate-200 rounded-xl space-y-1.5">
      ...
      {/* 📥 FIXED MOBILE FOOTER */}
      <footer className="md:hidden fixed bottom-0 left-0 w-full bg-white border-t border-slate-200 px-4 py-2 flex items-center justify-between gap-3 z-30 shadow-[0_-4px_14px_rgba(0,0,0,0.06)]">
        <button
          onClick={handleDownloadLocalDesktop}
          className="flex-1 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded-xl text-[11px] text-center transition"
        >
          📥 Download
        </button>
        <button
          onClick={handleSaveToDashboard}
          disabled={isSaving}
          className="flex-1 py-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white font-black rounded-xl text-[11px] text-center transition disabled:opacity-50"
        >
